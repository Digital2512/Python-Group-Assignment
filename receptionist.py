...
def pgReceptionist(userID):
    ...
    while True:
        ...
        if choice1 == 1:
            ...
            password = input("\nPassword: ")
            name = input("\nFull Name: ")
            print(f"User ID: {studentIDNumber}\n")
                ...
                for line in lines:
                    ...
                    if values[1] == searchStudentID:
                        ...
                        for deleteSubject in studentSubjects:
                            print(deleteSubject)
                            # Update the line with the modified list of subjects
                            deleteStudentList("SubjectsInfo.txt", deleteSubject, studentName)
                            
                        # Remove the subjects associated with the deleted student
            ...
            generalUtils.createNewLine()
            requestChoice = int(input("Choice: "))
            if requestChoice == 1:
                generalUtils.clearConsole()
                generalUtils.createNewLine()
                    ...
                    while True:
                        ...
                        if approval == "Y":
                            ...
                            if rejectSubjectChange("StudentRequest.txt", requestID, studentID, name, currentSubject, requestedSubject) == True:
                                print("Request Rejected(Subject Changed)")
                                generalUtils.createNewLine()
                                time.sleep(3)
                                break
                            generalUtils.createNewLine()
                ...
                if checkForValue("PaymentRequest.txt", paymentRequestID, 0) == True: 
                    ...
                    while True:
                        ...
                        if approval == "Y":
                            ...
                            receiptID = UniqueIDCreation.generateReceiptID(existingIDList)
                            if updateSubjectIncome("SubjectsInfo.txt", subjectCode, amount) and approvePayment("PaymentRequest.txt", paymentRequestID, studentID, name, subjectCode, amount, receiptID) == True:
                                receiptString = f"{receiptID};{studentID};{name};{subjectCode};{amount};APPROVED;"
                            ...
                            print("\nPayment Rejected")
                            if rejectPayment("PaymentRequest.txt", paymentRequestID, studentID, name, currentSubject, requestedSubject) == True:
                                receiptString = f"{receiptID};{studentID};{name};{subjectCode};{amount};REJECTED"
                                database.writeToFile("ReceiptCollection.txt", receiptString)
